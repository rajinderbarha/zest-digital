    ...
    <>
      ...
      <div className="  text-white mx-auto max-w-[1720px] w-full px-5">
        ...
        <div className=" font-mono text-55px border-white flex flex-col">
          <h1 className={`${classes.banner_text} font-semibold leading-tight border-white p-1 max-1810px:text-50px max-1350:text-45px  max-1350:pe-5  max-768:text-35px`}>{heading}</h1>
          <div className='flex gap-5 md:flex-col lg:flex-row mt-9 flex-col sm:flex-row'>

            <button className='text-base px-25px h-10 bg-white text-color-1 rounded-full hover:bg-color-1 hover:text-white border border-white  hover:border-color-1'>{viewSolBtnName}</button>
            <button className='text-base px-25px h-10 bg-transparent text-white rounded-full hover:text-color-1 hover:bg-white border border-white '>{SeeOurResBtnName}</button>
          </div>
