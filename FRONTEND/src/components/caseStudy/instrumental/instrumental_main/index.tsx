...
function Instrumental({ data, dynamicClass, singleClass }: { data: Gross_profitType, dynamicClass?: string, singleClass?: string }) {
  ...
  return <div className={`${classes.Instrumental} main_padding overflow-x-hidden`}>
    <div className={`${dynamicClass === "conversations" || dynamicClass === "project-window" ? `main_container` : ''} `}>
      <div className={`max-w-[1520px] mx-auto w-full     ${dynamicClass === "conversations" || dynamicClass === "project-window" ? `max-w-[1651px] me-auto mx-0` : ''} `}>
        <div className={`${dynamicClass !== "special-instrumental" && `grid sm:gap-10 sm:grid-cols-12`}  ${dynamicClass === "conversations" && classes.conversations_container}   ${dynamicClass === "project-window" && classes.project_window_container}  items-center`}>
          ...
          <div className={`${dynamicClass} ${dynamicClass !== "special-instrumental" && dynamicClass !== "conversations" && dynamicClass !== "project-window" && `sm:col-span-8 ms-auto lg:ps-[50px] md:ps-[30px] max-w-[891px] order-1 sm:order-2`}  ${dynamicClass === "conversations" && `${classes.conversations_text} md:col-span-6 ms-auto lg:ps-[50px] md:ps-[30px] max-w-[891px] order-1 md:order-2`}  ${dynamicClass === "project-window" && `${classes.project_window_text} `} ${dynamicClass === "project-window" && `${classes.project_window} md:col-span-6 ms-auto lg:pe-[50px] md:pe-[30px] max-w-[891px] order-1 md:order-1`} ${singleClass === "search-visits" && `${classes.search_visits_text}`} ${singleClass === "coversion-section" && `${classes.coversion_section}`} md:mt-0  `}
