...
const Header = ({ data }: { data: HeaderType[] }) => {
  ...
  <>
         {data?.map((item,index) => (
          <div key={index} className=' mx-auto flex justify-between items-center pt-25px pb-21px max-w-[1720px] w-full px-5 relative'>
        
       
      <div className='flex items-center gap-68px '>
        <div className='screen-1-max:w-24 lg:w-28 xl:w-auto'>
         
         <Link href={"/"}> <Image src={urlFor(item.logo).url()} alt="Site Logo"  width={112}
            height={51} />   </Link>

        </div>
        <div id="nav-menus" className={`${navMenu.menu.simple} + ${navMenu.menu.responsive1} + ${navMenu.menu.responsive2}`}>
        {item.navItem.map(item=>(  
          <Link href={item.buttonLink} key={`${index}`} className={navMenu.linkclass}>{item.buttonName}</Link>
            ))}
...
//       <div className='flex items-center gap-68px'>
//         <div className='w-24 lg:w-28 xl:w-auto'>
//           <Image src={logo} alt="Site Logo" />
//         </div>
//         <div id="nav-menus" className={`${navMenu.menu.simple} ${navMenu.menu.responsive1} ${navMenu.menu.responsive2}`}>
//           {/* Render nav items here */}
//         </div>
//       </div>
//       <div>
//         {/* Render schedule a call button and bars image here */}
//       </div>
//     </div>
//   );
// };
