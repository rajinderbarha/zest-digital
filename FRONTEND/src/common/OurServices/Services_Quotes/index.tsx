    ...
    <div className='main_container'>
      {Services_Quotes && Services_Quotes.map(({ desc_quote, name_quote }, index) => (
        <div className="" key={index}>
          {index % 2 == 0 ? (
            <div className={`${classes.quote_left} md:mt-0 sm:mt-10 mt-0`} data-aos="fade-right">
              <h2 className={`${classes.quote_heading} font-mono font-semibold min-tb:max-tb:leading-[37px] italic max-w-[1443px] `}>{desc_quote}</h2>
              <p className={`${classes.below_line} text-color-9  border-t font-mono  border-color-9`}>{name_quote}</p>
            </div>
          ) : (
            <div className={`${classes.quote_right} text-right  max-w-[1443px] ms-auto `} data-aos="fade-left">
